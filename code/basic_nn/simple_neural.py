...
class EssayGraderModel(LanguageModel):
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope('Layer') as hidden:
      W1 = tf.get_variable('W1', (self.config.embed_size, self.config.hidden_size), initializer=xavier_weight_init())
      b1 = tf.get_variable('b1', (self.config.hidden_size,), initializer=xavier_weight_init())
      h1 = tf.tanh(tf.matmul(window, W1)+ b1)
      with tf.variable_scope('Layer2') as hidden_2:
        ...
        with tf.variable_scope('Score') as score_scope:
          ...
          regularization = self.config.l2*0.5*(tf.reduce_sum(tf.square(W1)) + tf.reduce_sum(tf.square(W2)) + tf.reduce_sum(tf.square(U)))
  ...
  def add_loss_op(self, y):
    ...
    """
    ### YOUR CODE HERE
    reg = tf.get_collection("REGULARIZATION_LOSSES", scope='Layer/Layer2/Score')[0]
