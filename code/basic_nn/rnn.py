...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'), tf.name_scope("embedding_layer"):
      ...
      for x in tf.split(1, self.config.num_steps, self.input_placeholder):
        inputs.append(tf.nn.embedding_lookup(L, tf.squeeze(x, squeeze_dims=[1])))                                                        
      ### END YOUR CODE
      return inputs
  ...
  def add_projection(self, rnn_outputs):
    ...
    for i in range(self.config.num_steps):
      with tf.variable_scope("projection_layer") as scope:
        ...
        output = tf.matmul(rnn_outputs[i], U) + b_2
        outputs.append(tf.nn.dropout(output, self.dropout_placeholder))        
    ### END YOUR CODE
    return outputs
    ...
    for i in range(self.config.num_steps):
      with tf.variable_scope("rnn_layer") as scope:
        if i == 0:
          ...
          last_h = self.initial_state
        else:
          scope.reuse_variables()
          H = tf.get_variable("H")
          I = tf.get_variable("I")
          b_1 = tf.get_variable("b_1")
          last_h = rnn_outputs[i-1]
        # M1 = tf.matmul(last_h, H) 
        # M2 = tf.matmul(inputs[i], I) 
        # M3 = M1 + M2 
        # M4 = M3 + b_1
        h = tf.sigmoid(tf.matmul(last_h, H) + tf.matmul(inputs[i], I) + b_1)
